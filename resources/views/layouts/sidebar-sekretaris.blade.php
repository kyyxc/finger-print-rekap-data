        ...
        <aside id="sidebar" class="w-64 bg-slate-800 text-white flex flex-col fixed h-full z-50 transition-transform duration-300 -translate-x-full lg:translate-x-0 overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-700 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400">Sekretaris Panel</p>
                    </div>
                </div>
            ...
            <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
                <a href="{{ route('sekretaris.dashboard') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('sekretaris.dashboard') ? 'bg-blue-600 text-white' : 'hover:bg-slate-700 text-slate-300' }}">
                    <span class="material-icons text-xl">dashboard</span>
                    <span class="font-medium">Dashboard</span>
                </a>

                <a href="{{ route('sekretaris.users') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('sekretaris.users') ? 'bg-blue-600 text-white' : 'hover:bg-slate-700 text-slate-300' }}">
